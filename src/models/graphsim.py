    ...
    def __init__(self, kernel_size, stride, in_channels, out_channels, num_similarity_matrices):
        ...
        self.layers = torch.nn.ModuleList([
            torch.nn.Conv2d(in_channels=self.in_channels, out_channels=self.out_channels,
                            kernel_size=self.kernel_size, stride=stride) for _ in range(num_similarity_matrices)
        ])
...
class MaxPoolLayerV1(torch.nn.Module):
    def __init__(self, stride, pool_size, num_similarity_matrices):
        ...
        if self.pool_size % 2 == 0:
            ...
            self.padding = torch.nn.ZeroPad2d((padding_temp, padding_temp, padding_temp, padding_temp))
        self.layers = torch.nn.ModuleList([
            torch.nn.MaxPool2d(kernel_size=self.pool_size, stride=stride) for _ in range(num_similarity_matrices)
        ])
...
class GraphSim(torch.nn.Module):
    def __init__(
        ...
        num_ftrs = self.input_dim
        for i in range(self.num_gcn_layers):
            self.gnn_layers.append(self.get_gnn_layer(num_ftrs, gcn_size[i]))
            num_ftrs = gcn_size[i]
        ...
        for i in range(self.num_linear_layers-1):
            self.linear_layers.append(torch.nn.Linear(linear_size[i], linear_size[i+1]))
