...
import torch.nn as nn
import torch.nn.functional as F
from torch_geometric.nn import GINConv, GCNConv, GATConv, SAGEConv, ChebConv
from torch_geometric.data import Batch
from utils import model_utils
...
class EGSC(torch.nn.Module):
    def __init__(self, input_dim, filters, bottle_neck_neurons, dropout, gnn_layer, device, **kwargs):
        super(EGSC, self).__init__()
        self.device = device
        ...
        self.bottle_neck_neurons = bottle_neck_neurons
        self.dropout = dropout
        self.gnn_layer_type = gnn_layer

        self.feature_count = sum(self.filters) // 2

        self.gnn_layers = self.build_gnn_layers()

        self.attention_modules = nn.ModuleList([AttentionModule(f) for f in self.filters])
        self.tensor_network_modules = nn.ModuleList([SETensorNetworkModule(f) for f in self.filters])
    ...
    def build_gnn_layers(self):
        ...
        assert self.gnn_layer_type in gnn_layer_types, "Invalid GNN layer type"

        gnn_layer_class = gnn_layer_types[self.gnn_layer_type]
        ...
        for out_channels in self.filters:
            if self.gnn_layer_type == 'GIN':
                ...
                layers.append(gnn_layer_class(nn_seq, train_eps=True))
            elif self.gnn_layer_type == 'ChebNet':
                layers.append(gnn_layer_class(in_channels, out_channels, K=2))
            else:
                layers.append(gnn_layer_class(in_channels, out_channels))
    ...
    def forward(self, graphs, graph_sizes, graph_adj_matrices):
        ...
        for i, gnn_layer in enumerate(self.gnn_layers):
            features_1 = F.relu(gnn_layer(features_1, edge_index_1))
            features_1 = F.dropout(features_1, p=self.dropout, training=self.training)

            features_2 = F.relu(gnn_layer(features_2, edge_index_2))
            features_2 = F.dropout(features_2, p=self.dropout, training=self.training)

            pooled_features_1 = self.attention_modules[i](features_1, batch_1)
            pooled_features_2 = self.attention_modules[i](features_2, batch_2)

            pooled_features_list.append(self.tensor_network_modules[i](pooled_features_1, pooled_features_2))

        scores = torch.cat(pooled_features_list, dim=1)
        scores = F.relu(self.fully_connected_first(self.score_attention(scores) * scores + scores))
